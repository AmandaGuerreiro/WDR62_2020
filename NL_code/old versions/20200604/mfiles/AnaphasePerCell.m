function [AnaphaseA1,AnaphaseA2,AnaphaseB,ProbA1,ProbA2,ZenithA1,ZenithA2,MetaPhaseWidth] = AnaphasePerCell(Tracks)
...
d_ZK_2  = nan(length(TimeIdx),length(TrackID));%P2K2 projected on Zenital direction
MetaPhaseWidth = nan(length(TimeIdx),length(TrackID));%P1K2 projected on Zenital direction
for idxK = 1:length(TrackID)
...
%     
    ...
    PK = K_xyz-Pole1_xyz;
    MetaPhaseWidth(:,idxK) = dot(-u_Zenith,PK,2);
    %                 theta_1(idxK,idx) = acos(dot( u_AB(idx,:),PK,2)./d_PK);
end
...
MetaPhaseWidth = [MetaPhaseWidth,d_ZK_1];
tmp = array2table(MetaPhaseWidth);
MetaPhaseWidth = table(nanmean(MetaPhaseWidth,2),...
    nanmedian(MetaPhaseWidth,2),...
    nanstd(MetaPhaseWidth,1,2),...    
    skewness(MetaPhaseWidth,0,2),...    
    sum(~isnan(MetaPhaseWidth),2),...
    nanmin(MetaPhaseWidth,[],2),...
    nanmax(MetaPhaseWidth,[],2),...    
    TimeIdx);
MetaPhaseWidth.Properties.VariableNames = {'Mean','Median','StDev','Skewness','Nb','Min','Max','TimeIdx'};

tmp.Properties.VariableNames = cellfun(@num2str,num2cell([1:width(tmp)]),'UniformOutput',false);
MetaPhaseWidth = [MetaPhaseWidth tmp];
