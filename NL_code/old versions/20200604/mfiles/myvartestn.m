function [p,stats] = myvartestn(x,group,alpha,txttitle,varargin)
...
if dorobust==1 || dorobust==6 || dorobust==3
   ...
   if ngroups>1
       if dorobust==1 || dorobust==6
           [p,atab,stat] = anova1(xc.^2, group(t),'off');
           testname = getString(message('stats:vartestn:Levene'));
           ValTested = xc.^2;
       else
           [p,atab,stat] = anova1(abs(xc), group(t),'off');
   ...
   multic = NaN;
   if p<=alpha
       [c,m,h,gnames] = multcompare(stat,'Alpha',alpha,'Display','off');
       multic = c;
   end
...
ValTested = xcw;
   if ngroups>1
      [p,atab,stat] = anova1(xcw, group(t),'off');
   ...
   multic = NaN;
   if p<=alpha
       [c,m,h,gnames] = multcompare(atab,'Alpha',alpha,'Display','off');
       multic = c;
   end
...
elseif dorobust==5 
   ...
   if ngroups>1
      [p,atab,stat] = anova1(abs(xcbf), group(t),'off');
      ValTested = abs(xcbf);
      B = atab{2,5};                             % F statistic
      Bdf = [atab{2,3}, atab{3,3}];              % both d.f.
   else
...
if dodisplay
    ...
    if ~isnan(multic)
        try
            close(findall(groot,'type','figure','Name',[txttitle 'variance test']))
        end
        Table = cell(k+6+2+size(multic,1),6);
        Table(1,1:4) = { getString(message('stats:vartestn:Group')) ...
        ...
        Table{k+6,2} = p;        
        Table(k+8,1) = {'H0: mean difference is 0'};
        Table(k+8,3:6) = {'Lower CI','estimate','Upper CI','p'};
        Table(k+8+[1:size(multic,1)],1) = gname(multic(:,1));
        Table(k+8+[1:size(multic,1)],2) = gname(multic(:,2));
        
        Table(k+8+[1:size(multic,1)],3:6) = num2cell(multic(:,3:end));
        ...
        tblfig = statdisptable(Table, ...
         ...
         set(tblfig,'tag','table');
                set(tblfig,'Name',[txttitle ' variance test']);
    else
        Table = cell(k+6,4);
...
if (nargout > 1)
   stats = struct(statname,B,'df',Bdf,...
       'ValTested',ValTested,'Multicompare',multic);
end
...
try
    close(findall(groot,'type','figure','Name',txttitle))
end
