...
for i=1:m
    ...
    set(hB,'UpdateFcn',{@normplotDatatipCallback,slope(i),yinter(i),DATANAME});
...
if ~all(iscategorical(dataname))
datatipTxt{end+1} = sprintf('%s: %s',...
    getString(message('stats:probplot:Observation')),num2str(origind));
else
    datatipTxt{end+1} = sprintf('%s: %s',...
    getString(message('stats:probplot:Observation')),replace(char(dataname(origind)),'_','\_'));
end
if ~isempty(group)
    datatipTxt{end+1} = sprintf('%s: %s',...
